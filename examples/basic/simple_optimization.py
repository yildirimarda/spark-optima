...
def run_optimization(
    ...
    optimizer = Optimizer(platform=platform, spark_version=spark_version, optimization_mode="simulation")

    # Define resources
    resources = ResourceSpec(cpu_cores=16, memory_gb=max_memory_gb, disk_gb=500.0)
    ...
    data_profile = {
        ...
        "format": data_format,
        "compression": "snappy",
        "partitioning": {"type": "date", "column": "date"},
    }

    # Define resource constraints
    resource_constraints = {"max_memory_gb": max_memory_gb, "max_cost_usd": 50.0, "max_executors": 20}
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Spark Optima - Simple Optimization Example")
    ...
    parser.add_argument("--spark-version", type=str, default="3.5.0", help="Spark version (default: 3.5.0)")

    parser.add_argument("--data-size", type=float, default=100.0, help="Data size in GB (default: 100)")
    ...
    parser.add_argument("--max-memory", type=float, default=64.0, help="Maximum memory in GB (default: 64)")

    parser.add_argument("--no-bayesian", action="store_true", help="Disable Bayesian optimization")

    parser.add_argument("--trials", type=int, default=30, help="Number of Bayesian trials (default: 30)")
