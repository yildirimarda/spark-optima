...
class TestGetPlatform:
    ...
    def test_get_platform_invalid_name_includes_valid(self) -> None:
        """Test error message includes valid platform names."""
        with pytest.raises(ValueError, match="local") as exc_info:
            get_platform("nonexistent")
        assert "aws_glue" in str(exc_info.value)
        assert "databricks" in str(exc_info.value)


class TestListPlatforms:
    """Test cases for list_platforms function."""
...
class TestGetPlatformInfo:
    ...
    def test_get_platform_info_handles_exceptions(self, mock_registry: MagicMock) -> None:
        """Test that get_platform_info handles exceptions gracefully."""
        # Create a mock that raises an exception when called
        mock_platform_class = MagicMock(side_effect=Exception("Test error"))
