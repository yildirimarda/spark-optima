...
- [x] Spot/preemptible strategy advisor: recommend spot-vs-on-demand executor mix per platform with expected interruption cost modeled from the workload's stage retry tolerance
...
- [x] Spark Connect support: run trials against a remote Spark Connect endpoint (the Spark 4.x default) instead of a local session, so tuning works against real clusters without shipping code
